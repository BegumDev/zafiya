...
form.addEventListener('submit', function(ev) {
    ...
    card.update({
        'disabled': true
    }); // disable to avoid a double payment
...
// form.addEventListener('submit', function(ev) {
//     ev.preventDefault();
//     card.update({'disabled': true}); // disable to avoid a double payment
//     $('#submit-button').attr('disabled', true);
...
//                 var html = `<span role="alert">${result.error.message}</span>`;
//                 $(errorDiv).html(html);
//                 card.update({'disabled': false}); // enable to allow them to fix it
//                 $('#submit-button').attr('disabled', false);
//             } else {
//                 if (result.paymentIntent.status === 'succeeded') {
//                     form.submit();
...
//         });
//     }).fail(function () {
//         // reload the page and the error will display in django messages
//         location.reload()
//     });
// });
